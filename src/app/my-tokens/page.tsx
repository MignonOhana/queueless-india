...
import { 
  ChevronLeft, Ticket, Clock, Building2, 
...
export default function MyTokensPage() {
  ...
  const fetchGuestTokens = useCallback(() => {
      ...
      const enriched: TokenRecord[] = await Promise.all(
        (tokens || []).map(async (t: any) => {
          const { data: biz } = await (supabase
            ...
            .eq("id", t.orgId)
            .maybeSingle();
          return { ...t, businessName: biz?.name || t.orgId, business_id: t.orgId };
        })
      );
  ...
  }, [isAuthenticated, loading, fetchTokens, fetchGuestTokens]);
  ...
  const TokenCard = ({ token }: { token: TokenRecord }) => {
    ...
    return (
      <motion.div
        ...
        className={`bg-[#111118] border border-white/5 rounded-2xl p-5 flex items-center gap-4 transition-all ${isActive ? "hover:border-primary/30 hover:bg-[#111118]/80 cursor-pointer active:scale-[0.98]" : ""}`}
      >
        {/* Token Icon */}
        ...
        <div className="flex items-center gap-2">
          {isActive ? (
             <button 
                onClick={(e) => {
                  e.stopPropagation();
                  router.push(`/track/${token.tokenNumber}`);
                }}
            ...
            <button 
              onClick={(e) => {
                e.stopPropagation();
                if (token.business_id) router.push(`/customer/business/${token.business_id}`);
              }}
              className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-white/10 transition-colors"
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] text-white font-sans selection:bg-primary/30 selection:text-primary">
      {/* Background Accent */}
      <div className="fixed top-0 inset-x-0 h-[40vh] bg-gradient-to-b from-primary/5 via-transparent to-transparent pointer-events-none" />
      ...
      <main className="relative z-10 max-w-lg mx-auto px-5 pt-12 pb-24">
        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm font-medium">Your queue history</p>
          </div>
        </div>

        {/* Tabs */}
        <div className="flex bg-[#111118] border border-white/5 rounded-2xl p-1.5 mb-8 overflow-hidden">
          {(["active", "past"] as TabType[]).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === tab ? "bg-primary text-[#0A0A0F] shadow-lg shadow-primary/20" : "text-zinc-500 hover:text-white"}`}
            >
            ...
            {activeTab === "active" ? (
              ...
              >
                {activeTokens.length === 0 ? (
                  <div className="text-center py-20 bg-white/[0.02] border border-dashed border-white/10 rounded-[2rem] p-8">
                    <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center mx-auto mb-4">
                      <Ticket size={28} className="text-zinc-600" />
                    </div>
                    <p className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">No active tokens</p>
                    <p className="text-zinc-600 text-xs mt-2 font-medium">Join a queue at a nearby business!</p>
                    <button
                      onClick={() => router.push("/home")}
                      className="mt-8 px-8 py-4 bg-primary text-black font-black uppercase tracking-widest text-[10px] rounded-2xl shadow-lg shadow-primary/20 hover:brightness-110 active:scale-95 transition-all"
                    >
                      Browse Businesses
                    </button>
                  </div>
              ...
              >
                {pastTokens.length === 0 ? (
                  <div className="text-center py-20 bg-white/[0.02] border border-dashed border-white/10 rounded-[2rem] p-8">
                    ...
                    <p className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">No past visits yet</p>
                    <p className="text-zinc-600 text-xs mt-2 font-medium">Your completed tokens will appear here</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-4">
